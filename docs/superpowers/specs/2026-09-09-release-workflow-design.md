...
**Status:** Implemented — verified end-to-end on 2026-09-09 with tags `v0.0.1-ci.1` and `v0.0.1.1`, both since deleted. The pre-release flag guard added afterwards (see the last Amendments entry) is verified locally against a fake `gh` only; no further tag was pushed for it
...
| Re-run behavior | Create-or-update: an existing release gets assets uploaded with `--clobber`, and the run then fails if that release is a draft or if its pre-release flag contradicts the tag |
...
- **Publishing a release from the GitHub UI first**, which creates the tag and fires this workflow, results in the workflow attaching packages to that release and leaving the hand-written notes untouched, provided its pre-release checkbox agrees with the tag.
...
- **The update path changes nothing but assets.** If an existing release was hand-created with the wrong title, the workflow does not correct it.
- **A pre-release flag that contradicts the tag is an error, not a correction.** A release hand-created in the UI for a tag such as `v1.0.0-rc.1` with the pre-release box left unticked would otherwise take the update path, receive the packages, and exit 0, leaving an `-rc` build standing as the repository's latest stable release with nothing red in the run. The probe reads `isPrerelease` in the same call that reads `isDraft` and the job fails after attaching assets, naming both the release flag and the value the tag requires. Failing rather than calling `gh release edit --prerelease` keeps the contract above: this path writes assets and nothing else. Correcting the flag by hand, or deleting the release and re-running, is a deliberate manual step, exactly as for a draft.
- **A draft whose flag is also wrong reports the draft first.** The draft test precedes the flag test, so publishing the draft and re-running surfaces the flag mismatch on the second run rather than both at once. Both are red, so neither can be missed.
...
| An existing release's pre-release flag contradicts the tag | `release` step 2 | Assets are attached, then the run fails with a message naming `isPrerelease` and the value the tag requires. Nothing is published as stable that should not be. Fix the flag on the release, or delete the release, then re-run the `release` job. |
...
Carried out by `docs/superpowers/plans/2026-09-09-release-workflow.md`. All six checks passed; results recorded below.
...
6. **Release job branches.** A harness extracts the shipped `Create or update release` step out of the YAML and executes it against a fake `gh` on `PATH` that logs its arguments and answers from environment variables, covering ten cases: an existing published release whose flag matches in both directions, an existing published release whose flag is wrong in both directions, an existing draft with a matching and with a wrong flag, the create path with and without `--prerelease`, a stale empty `PRERELEASE`, and an empty asset set. All ten behave as documented. Four mutations were each caught: deleting the flag branch turned the two mismatch cases green-with-assets-attached, reproducing the original defect; inverting the comparison turned four cases red; changing the probe filter tripped the fake, which drops every update case through to the create path and so also shows the failure direction if `gh` ever errored on the filter, a red run from `already_exists` rather than a mispublish; and the pre-fix workflow failed the harness's extraction guard. The fake never reaches the network, so this check pushes no tag. This check was added after the end-to-end runs below and is the only verification the pre-release flag guard has; it has not been exercised against a real GitHub release.
...
**2026-09-09, after a review of the `release` job update path.** The complete workflow is 248 lines. Verified locally only: actionlint 1.7.12 with shellcheck 0.11.0 confirmed active through `-verbose`, and the ten-case release-step harness described under Verification. No tag was pushed for this round.

| Change | Reason |
|---|---|
| The existence probe captures `isPrerelease` alongside `isDraft`, and the update path fails if it disagrees with `$PRERELEASE` | The update path never read the existing release's pre-release flag. A published release hand-created in the UI for a tag like `v1.0.0-rc.1` with the box unticked took that path, received the packages, and exited 0, leaving an `-rc` build as the repository's latest stable release with nothing red in the run. The create path had always passed `--prerelease`, and the job already rejected a `$PRERELEASE` that was not exactly `true` or `false`, so the update path was the only hole. Fourth instance of the same silent-success class, after the empty `PRERELEASE`, `IsPackable=false`, and the accepted draft |
| Failing rather than calling `gh release edit --prerelease` | Keeps the documented contract that the update path changes nothing but assets. Correcting the flag would make the job a writer of release metadata, and a release whose flag was set deliberately by a human would then be silently overwritten by a re-run |
| Probe filter is `--jq '.isDraft, .isPrerelease'`, split with two parameter expansions, rather than `--jq '[...] | @tsv'` | The comma form renders each scalar exactly as the already-proven `--jq .isDraft` and `--jq .url` calls do, so it adds no assumption about how the `gh` bundled jq formats a boolean inside a format string, which cannot be checked locally without `gh` reaching the API. If the output is ever not two lines, both variables become the whole string, which fails the flag comparison rather than mispublishing |
| Draft test kept ahead of the flag test | A draft is the more urgent condition and its message names the one action that unblocks the release. The consequence, that a draft with a wrong flag reports the flag only on the re-run, is documented above rather than removed |

Considered and not adopted: reporting both the draft and the flag mismatch in a single run, which would mean collecting problems into an array and emitting them together, restructuring a branch that is otherwise a plain sequence of guards for a case that is already red either way. Also still open from the Chunk 3 review, and unchanged by this round: the probe discards the `gh release view` diagnostic, so an auth or rate-limit failure is reported as `already_exists` from the create step rather than as the real cause, and `isImmutable` is available in the same probe and would let the job name immutable releases as the reason `upload --clobber` was refused.
