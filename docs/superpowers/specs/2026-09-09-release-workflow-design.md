...
- ~~**Amend `2026-05-22-repo-restructure-design.md` on three points**~~ **Done on 2026-09-09** in commit `5551043`, which corrected four `Microsoft.Data.Sqlite` literals in `2026-05-22-repo-restructure.md` and one in its design doc, and added a warning note at the top of that plan. The three points, each of which would otherwise have turned every tag run red once the restructure lands:
