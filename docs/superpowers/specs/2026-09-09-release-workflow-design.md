...
| 9 | Verify packages | Fails if `artifacts/packages` contains no `.nupkg`. Fails if the number of `.nupkg` files differs from the number of `src/*/*.csproj` projects, which is how a project that produced no package becomes a red run rather than a silent omission. For every package file present, fails unless the filename ends in `.$RELEASE_VERSION.nupkg` or `.$RELEASE_VERSION.snupkg`, which proves the version override took effect. Fails if `find src -mindepth 3 -name '*.csproj'` returns anything, which means a project was nested too deep for step 8's glob and would otherwise be dropped from the release on a green run. Writes the resulting filenames to `$GITHUB_STEP_SUMMARY`; the packed-project list is written by step 8 as each pack succeeds. |
...
| `gh release view` fails for a reason other than the release not existing, such as a rate limit | `release` step 2 | The probe falls through to the create path, which then fails because the release exists. The run is red and nothing is mispublished, but the log points at the wrong cause. |
